...
import { CheckCheck, Icon, LucideIcon } from "lucide-react";
    ...
    <main className="flex min-h-screen h-fit flex-col items-center justify-center relative">
      ...
      >
        ...
        <div className="w-full  h-full  flex items-center justify-center relative -z-10">
           <Spline
            className="w-full flex scale-[.20] sm:scale-[.35] lg:scale-[.5] items-center justify-center md:justify-start"
      ...
      <section
        id="about"
        className="h-fit min-h-screen w-full flex items-center justify-center p-8"
      >
        <div className="absolute -z-10 h-full w-full overflow-hidden">
          <Image
            src="/whirl.svg"
            fill
            className="absolute object-contain w-full overflow-visible sm:rotate-90 "
      ...
      <section id="pricing" className="h-fit min-h-screen w-full flex flex-col items-center justify-center p-8">
        <h4 className="text-4xl md:text-5xl font-bold"> Pricing that fits your needs</h4>
       <div className="grid grid-cols-1 grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 items-center h-fit w-full max-w-3xl gap-8">
        {pricingCards.map((pricingCards) => {
          return (
         <PricingCard onelinee={pricingCards.oneliner} title={pricingCards.title} price={pricingCards.price} benefits={pricingCards.benefits} key={pricingCards.id}/>
          )
        })}
       </div>
      </section>
  ...
  <div className="h-fit w-full rounded flex flex-col p-8 gap-8 bg-gray-900 rounded bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-20">
    ...
    <div className="flex flex-col w-full gap-4">
      {benefits.map((benefits, i) => { 
        return(
          <p className="text-sm text-zinc-500 flex items-center gap-2" key={i}>
            <span>
              <CheckCheck/>
            </span>
            {benefits}
